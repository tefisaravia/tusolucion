...
class AgentPress_Listings {
	...
	public function __construct() {
		...
		$this->property_details = apply_filters(
			...
			array(
				'col1' => array(
					__( 'Precio:', 'agentpress-listings' ) => '_listing_precio',
					__( 'Dormitorio:', 'agentpress-listings' ) => '_listing_dormitorio',
					__( 'Categoria:', 'agentpress-listings' )  => '_listing_categoria',
					__( 'Tipo:', 'agentpress-listings' ) => '_listing_tipo',
					__( 'Zona:', 'agentpress-listings' )   => '_listing_zona',
				),
				'col2' => array(
					__( 'Metros:', 'agentpress-listings' ) => '_listing_metros',
					__( 'Estacionamiento:', 'agentpress-listings' ) => '_listing_estacionamiento',
					__( 'Codigo:', 'agentpress-listings' ) => '_listing_codigo',
					__( 'Baños:', 'agentpress-listings' ) => '_listing_baños',
					__( 'Compartido:', 'agentpress-listings' ) => '_listing_compartido',
				),
			)
		);
	...
	public function metabox_save( $post_id, $post ) {
		...
		if ( isset( $property_details['_listing_precio'] ) && ! empty( $property_details['_listing_precio'] ) ) {
			$precio_sortable = preg_replace( '/[^0-9\.]/', '', $property_details['_listing_precio'] );
			update_post_meta( $post_id, '_listing_precio_sortable', floatval( $precio_sortable ) );
		} else {
			delete_post_meta( $post_id, '_listing_precio_sortable' );
		}
	}
	...
	public function property_details_shortcode( $atts ) {
		...
		$output .= sprintf( '<p><b>%s</b><br /> %s</p></div>', __( 'Otros aspectos:', 'agentpress-listings' ), get_the_term_list( $post->ID, 'features', '', ', ', '' ) );
